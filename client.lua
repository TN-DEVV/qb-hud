...
local stress = 0
local hunger = 100
local addictmeth = 100
local addictcoke = 100
local thirst = 100
local cashAmount = 0
local bankAmount = 0
local nitroActive = 0
...
RegisterNUICallback('dynamicAddictmeth', function()
    Wait(50)
    Menu.isDynamicAddictmethChecked = not Menu.isDynamicAddictmethChecked
    TriggerEvent("hud:client:playHudChecklistSound")
    saveSettings()
end)

RegisterNUICallback('dynamicAddictcoke', function()
    Wait(50)
    Menu.isDynamicAddictcokeChecked = not Menu.isDynamicAddictcokeChecked
    TriggerEvent("hud:client:playHudChecklistSound")
    saveSettings()
end)
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst, newAddictMeth, newAddictCoke) -- Triggered in qb-core
    hunger = newHunger
    thirst = newThirst
    addictmeth = newAddictMeth
    addictcoke = newAddictCoke
end)
...
local function updatePlayerHud(data)
    ...
    if shouldUpdate then
        SendNUIMessage({
            ...
            dynamicAddictmeth = data[6],
            dynamicAddictcoke = data[7],
            dynamicStress = data[8],
            dynamicOxygen = data[9],
            dynamicEngine = data[10],
            dynamicNitro = data[11],
            health = data[12],
            playerDead = data[13],
            armor = data[14],
            thirst = data[15],
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            updatePlayerHud({
                ...
                Menu.isDynamicAddictmethChecked,
                Menu.isDynamicAddictcokeChecked,
